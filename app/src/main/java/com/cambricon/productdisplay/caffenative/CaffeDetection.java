...
public class CaffeDetection {
    private static String TAG = "CaffeDetection";
    ...
    private native float[] detection(byte[] bitmap, int channels, float[]mean);
    public native void hello();

    public float[] detectionImage(String fileName, float[] mean) {
        CaffeImage image = readImage(fileName);
        float[] result = detection(image.pixels, image.channels, mean);
        return result;
    }
    class CaffeImage {
        int channels;
